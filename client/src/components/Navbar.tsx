            ...
            >
               ...
               <img src="/logo.svg" alt="logo"  className="h-8.5 w-auto"/></Link>
                ...
                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    ...
                    <Link to='/my-generations' className="hover:text:pink-300 transition">My Generations</Link>
                    :
...
<div className="relative group">
    ...
    <div className="absolute hidden group-hover:block top-6 right-0 pt-4">
        <button className="bg-white/20 border-2 border-white/10 px-5 py-1.5 rounded" onClick={()=>logOut()}>
            Logout
        </button>
...
<button onClick={() => setIsOpen(true)} className="md:hidden">
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
               ...
               <Link onClick={() => setIsOpen(false)} to='#' >About</Link>
                ...
                {isLoggedIn?(
                    <button onClick={()=>{setIsOpen(false);logOut();}} className="px-6 py-2.5 bg-pink-600 hover:bg-pink-700 active:scale-95 transition-all rounded-full">
                    Logout
                </button>
                ):
